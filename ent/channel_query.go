...
type ChannelQuery struct {
	...
	withSubscriptions      *ChannelSubQuery
	withMessages           *ChannelMessageQuery
	loadTotal              []func(context.Context, []*Channel) error
	modifiers              []func(*sql.Selector)
	withNamedSubscriptions map[string]*ChannelSubQuery
...
func (cq *ChannelQuery) QuerySubscriptions() *ChannelSubQuery {
	query := (&ChannelSubClient{config: cq.config}).Query()
...
func (cq *ChannelQuery) QueryMessages() *ChannelMessageQuery {
	query := (&ChannelMessageClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *ChannelQuery) WithSubscriptions(opts ...func(*ChannelSubQuery)) *ChannelQuery {
	query := (&ChannelSubClient{config: cq.config}).Query()
...
func (cq *ChannelQuery) WithMessages(opts ...func(*ChannelMessageQuery)) *ChannelQuery {
	query := (&ChannelMessageClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *ChannelQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Channel, error) {
	...
	if query := cq.withSubscriptions; query != nil {
		if err := cq.loadSubscriptions(ctx, query, nodes,
			func(n *Channel) { n.Edges.Subscriptions = []*ChannelSub{} },
			func(n *Channel, e *ChannelSub) { n.Edges.Subscriptions = append(n.Edges.Subscriptions, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range cq.withNamedSubscriptions {
		if err := cq.loadSubscriptions(ctx, query, nodes,
			func(n *Channel) { n.appendNamedSubscriptions(name) },
			func(n *Channel, e *ChannelSub) { n.appendNamedSubscriptions(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range cq.withNamedMessages {
		if err := cq.loadMessages(ctx, query, nodes,
...
func (cq *ChannelQuery) loadSubscriptions(ctx context.Context, query *ChannelSubQuery, nodes []*Channel, init func(*Channel), assign func(*Channel, *ChannelSub)) error {
	...
	query.Where(predicate.ChannelSub(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(channel.SubscriptionsColumn), fks...))
	}))
...
func (cq *ChannelQuery) loadMessages(ctx context.Context, query *ChannelMessageQuery, nodes []*Channel, init func(*Channel), assign func(*Channel, *ChannelMessage)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Channel)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (cq *ChannelQuery) WithNamedSubscriptions(name string, opts ...func(*ChannelSubQuery)) *ChannelQuery {
	query := (&ChannelSubClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if cq.withNamedSubscriptions == nil {
		cq.withNamedSubscriptions = make(map[string]*ChannelSubQuery)
	}
	cq.withNamedSubscriptions[name] = query
	return cq
}

// WithNamedMessages tells the query-builder to eager-load the nodes that are connected to the "messages"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (cq *ChannelQuery) WithNamedMessages(name string, opts ...func(*ChannelMessageQuery)) *ChannelQuery {
	query := (&ChannelMessageClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if cq.withNamedMessages == nil {
		cq.withNamedMessages = make(map[string]*ChannelMessageQuery)
	}
	cq.withNamedMessages[name] = query
	return cq
}
