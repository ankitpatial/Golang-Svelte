...
type ProductQuery struct {
	...
	withPackage      *ProductPackageQuery
	withCreator      *UserQuery
	withImage        *DocumentQuery
	withFKs          bool
	loadTotal        []func(context.Context, []*Product) error
	modifiers        []func(*sql.Selector)
	withNamedPackage map[string]*ProductPackageQuery
...
func (pq *ProductQuery) QueryPackage() *ProductPackageQuery {
	query := (&ProductPackageClient{config: pq.config}).Query()
...
func (pq *ProductQuery) QueryCreator() *UserQuery {
...
func (pq *ProductQuery) QueryImage() *DocumentQuery {
...
func (pq *ProductQuery) Clone() *ProductQuery {
	...
	return &ProductQuery{
		...
		withPackage: pq.withPackage.Clone(),
...
func (pq *ProductQuery) WithPackage(opts ...func(*ProductPackageQuery)) *ProductQuery {
	query := (&ProductPackageClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withPackage = query
	return pq
}

// WithCreator tells the query-builder to eager-load the nodes that are connected to
// the "creator" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithCreator(opts ...func(*UserQuery)) *ProductQuery {
	query := (&UserClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProductQuery) WithImage(opts ...func(*DocumentQuery)) *ProductQuery {
	query := (&DocumentClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if pq.withCreator != nil || pq.withImage != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withPackage; query != nil {
		if err := pq.loadPackage(ctx, query, nodes,
			func(n *Product) { n.Edges.Package = []*ProductPackage{} },
			func(n *Product, e *ProductPackage) { n.Edges.Package = append(n.Edges.Package, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withCreator; query != nil {
		if err := pq.loadCreator(ctx, query, nodes, nil,
			func(n *Product, e *User) { n.Edges.Creator = e }); err != nil {
			return nil, err
		}
	}
	if query := pq.withImage; query != nil {
		if err := pq.loadImage(ctx, query, nodes, nil,
			func(n *Product, e *Document) { n.Edges.Image = e }); err != nil {
			return nil, err
		}
	}
	for name, query := range pq.withNamedPackage {
		if err := pq.loadPackage(ctx, query, nodes,
			func(n *Product) { n.appendNamedPackage(name) },
			func(n *Product, e *ProductPackage) { n.appendNamedPackage(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *ProductQuery) loadPackage(ctx context.Context, query *ProductPackageQuery, nodes []*Product, init func(*Product), assign func(*Product, *ProductPackage)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(product.PackageTable)
		s.Join(joinT).On(s.C(productpackage.FieldID), joinT.C(product.PackagePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(product.PackagePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(product.PackagePrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*ProductPackage](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "package" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (pq *ProductQuery) loadCreator(ctx context.Context, query *UserQuery, nodes []*Product, init func(*Product), assign func(*Product, *User)) error {
...
func (pq *ProductQuery) loadImage(ctx context.Context, query *DocumentQuery, nodes []*Product, init func(*Product), assign func(*Product, *Document)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Product)
	for i := range nodes {
...
// WithNamedPackage tells the query-builder to eager-load the nodes that are connected to the "package"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithNamedPackage(name string, opts ...func(*ProductPackageQuery)) *ProductQuery {
	query := (&ProductPackageClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if pq.withNamedPackage == nil {
		pq.withNamedPackage = make(map[string]*ProductPackageQuery)
	}
	pq.withNamedPackage[name] = query
	return pq
}
