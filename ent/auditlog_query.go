...
func (alq *AuditLogQuery) QueryAPIUser() *ApiUserQuery {
	query := (&ApiUserClient{config: alq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := alq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (alq *AuditLogQuery) Clone() *AuditLogQuery {
	...
	return &AuditLogQuery{
		...
		withAPIUser: alq.withAPIUser.Clone(),
...
func (alq *AuditLogQuery) WithAPIUser(opts ...func(*ApiUserQuery)) *AuditLogQuery {
	query := (&ApiUserClient{config: alq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	alq.withAPIUser = query
	return alq
}
...
func (alq *AuditLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AuditLog, error) {
	...
	if alq.withUser != nil || alq.withAPIUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := alq.withAPIUser; query != nil {
		if err := alq.loadAPIUser(ctx, query, nodes, nil,
			func(n *AuditLog, e *ApiUser) { n.Edges.APIUser = e }); err != nil {
			return nil, err
		}
	}
...
func (alq *AuditLogQuery) loadAPIUser(ctx context.Context, query *ApiUserQuery, nodes []*AuditLog, init func(*AuditLog), assign func(*AuditLog, *ApiUser)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*AuditLog)
	for i := range nodes {
...
func (alq *AuditLogQuery) Modify(modifiers ...func(s *sql.Selector)) *AuditLogSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (als *AuditLogSelect) Modify(modifiers ...func(s *sql.Selector)) *AuditLogSelect {
