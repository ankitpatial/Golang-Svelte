...
type UserSessionQuery struct {
	...
	withSockets        *UserSessionSocketQuery
	withUser           *UserQuery
	withPartner        *PartnerQuery
	withPartnerContact *PartnerContactQuery
	withFKs            bool
	loadTotal          []func(context.Context, []*UserSession) error
	modifiers          []func(*sql.Selector)
	withNamedSockets   map[string]*UserSessionSocketQuery
...
// QuerySockets chains the current query on the "sockets" edge.
func (usq *UserSessionQuery) QuerySockets() *UserSessionSocketQuery {
	query := (&UserSessionSocketClient{config: usq.config}).Query()
...
func (usq *UserSessionQuery) QueryPartner() *PartnerQuery {
	query := (&PartnerClient{config: usq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := usq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryPartnerContact chains the current query on the "partner_contact" edge.
func (usq *UserSessionQuery) QueryPartnerContact() *PartnerContactQuery {
	query := (&PartnerContactClient{config: usq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := usq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (usq *UserSessionQuery) Clone() *UserSessionQuery {
	...
	return &UserSessionQuery{
		...
		withSockets:        usq.withSockets.Clone(),
		withUser:           usq.withUser.Clone(),
		withPartner:        usq.withPartner.Clone(),
		withPartnerContact: usq.withPartnerContact.Clone(),
...
// WithSockets tells the query-builder to eager-load the nodes that are connected to
// the "sockets" edge. The optional arguments are used to configure the query builder of the edge.
func (usq *UserSessionQuery) WithSockets(opts ...func(*UserSessionSocketQuery)) *UserSessionQuery {
	query := (&UserSessionSocketClient{config: usq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	usq.withSockets = query
	return usq
}
...
func (usq *UserSessionQuery) WithPartner(opts ...func(*PartnerQuery)) *UserSessionQuery {
	query := (&PartnerClient{config: usq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	usq.withPartner = query
	return usq
}

// WithPartnerContact tells the query-builder to eager-load the nodes that are connected to
// the "partner_contact" edge. The optional arguments are used to configure the query builder of the edge.
func (usq *UserSessionQuery) WithPartnerContact(opts ...func(*PartnerContactQuery)) *UserSessionQuery {
	query := (&PartnerContactClient{config: usq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	usq.withPartnerContact = query
	return usq
}
...
func (usq *UserSessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserSession, error) {
	var (
		...
		loadedTypes = [4]bool{
			...
			usq.withUser != nil,
			usq.withPartner != nil,
			usq.withPartnerContact != nil,
		}
	)
	if usq.withUser != nil || usq.withPartner != nil || usq.withPartnerContact != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := usq.withSockets; query != nil {
		if err := usq.loadSockets(ctx, query, nodes,
			func(n *UserSession) { n.Edges.Sockets = []*UserSessionSocket{} },
			func(n *UserSession, e *UserSessionSocket) { n.Edges.Sockets = append(n.Edges.Sockets, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := usq.withPartner; query != nil {
		if err := usq.loadPartner(ctx, query, nodes, nil,
			func(n *UserSession, e *Partner) { n.Edges.Partner = e }); err != nil {
			return nil, err
		}
	}
	if query := usq.withPartnerContact; query != nil {
		if err := usq.loadPartnerContact(ctx, query, nodes, nil,
			func(n *UserSession, e *PartnerContact) { n.Edges.PartnerContact = e }); err != nil {
			return nil, err
		}
	}
	for name, query := range usq.withNamedSockets {
		if err := usq.loadSockets(ctx, query, nodes,
			func(n *UserSession) { n.appendNamedSockets(name) },
			func(n *UserSession, e *UserSessionSocket) { n.appendNamedSockets(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (usq *UserSessionQuery) loadSockets(ctx context.Context, query *UserSessionSocketQuery, nodes []*UserSession, init func(*UserSession), assign func(*UserSession, *UserSessionSocket)) error {
	...
	query.withFKs = true
	query.Where(predicate.UserSessionSocket(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(usersession.SocketsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.sessions_id
		if fk == nil {
			return fmt.Errorf(`foreign-key "sessions_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "sessions_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (usq *UserSessionQuery) loadPartner(ctx context.Context, query *PartnerQuery, nodes []*UserSession, init func(*UserSession), assign func(*UserSession, *Partner)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*UserSession)
	for i := range nodes {
...
func (usq *UserSessionQuery) loadPartnerContact(ctx context.Context, query *PartnerContactQuery, nodes []*UserSession, init func(*UserSession), assign func(*UserSession, *PartnerContact)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*UserSession)
	for i := range nodes {
		if nodes[i].partner_contact_id == nil {
			continue
		}
		fk := *nodes[i].partner_contact_id
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(partnercontact.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "partner_contact_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
// WithNamedSockets tells the query-builder to eager-load the nodes that are connected to the "sockets"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (usq *UserSessionQuery) WithNamedSockets(name string, opts ...func(*UserSessionSocketQuery)) *UserSessionQuery {
	query := (&UserSessionSocketClient{config: usq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if usq.withNamedSockets == nil {
		usq.withNamedSockets = make(map[string]*UserSessionSocketQuery)
	}
	usq.withNamedSockets[name] = query
	return usq
}
