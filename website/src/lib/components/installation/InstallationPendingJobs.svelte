<script lang='ts'>
  ...
  import { QryPendingInstallations } from '$lib/gql/installation';
  ...
  import DDLApproval from './DDLApproval.svelte';
...
<Page {title}>
  <svelte:fragment slot='filters'>
    <div class='flex flex-col sm:flex-row gap-x-2 justify-start'>
      ...
      <DDLApproval bind:value={status} />
    </div>
  </svelte:fragment>

  <DataTable
    query={QryPendingInstallations}
    dataProp='pendingInstallations'
    pageSize={20}
    headers={headers}
    {variables}
    searchable
  ...
  >
    <svelte:fragment slot='row' let:item let:headers>
      ...
      <DataColumn header={headers[5]}>{localDate(item.createdAt)}</DataColumn>
      {#if (isAdmin)}
